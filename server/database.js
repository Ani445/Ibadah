...
function loadPost(callback) {
    const sql = `SELECT p.post_id,
                        ...
                        p. date,
                        p. time,
                        com.user_name, com.comment_text, com.date, com.time
                 FROM posts p, comments com where p.post_id= com.post_id
                 order by post_id desc`;
...
function insertNewComments(postID, topic, callback) {
    ...
    const sql = `INSERT INTO comments(post_id, user_id, user_name, comment_text, date, time)
                 VALUES (${pool.escape(postID)}, ${pool.escape(id_user)}, ${pool.escape(name_user)}, ${pool.escape(topic)},
                 ${pool.escape(date)}, ${pool.escape(time)})`;
    pool.query(sql, (err) => {
        if (err) {
            console.log(err.sqlMessage + '\n' + err.sql);
...
function loadDuaByCategory(category, callback) {
    const query = `SELECT dua_id, title, dua_text, meaning, source from dua WHERE cat_id = ${pool.escape(category)} order by dua_id asc`;
